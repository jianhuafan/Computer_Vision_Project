...
im = cv2.imread('../results/output1.png')
...
open_kernel = np.ones((1, 4), np.uint8)
im = cv2.morphologyEx(im, cv2.MORPH_OPEN, open_kernel)
...
cv2.floodFill(im_copy, mask, (h/3,w/3), 255)
...
cv2.imwrite('../results/output1.png', output)
cv2.waitKey(0)